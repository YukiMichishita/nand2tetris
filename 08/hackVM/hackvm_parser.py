...
Commands = {
    ...
    CommandType.C_LABEL: Command(fr'(label)\s*[^\d\s][^\s]*', 1),
    CommandType.C_GOTO: Command(r'(goto)\s*[^\d\s][^\s]*', 1),
    CommandType.C_IF: Command(r'(if-goto)\s*[^\d\s][^\s]*', 1),
    CommandType.C_FUNCTION: Command(fr'(function)\s*(.*)\s*\d+', 2),
    CommandType.C_RETURN: Command(r'return\s*', 0),
    CommandType.C_CALL: Command(r'(call)\s*(.*)\s*\d+', 2),
